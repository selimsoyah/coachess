...
import { getPlayerAssignments, type AssignmentWithDetails } from '@/lib/assignments/assignments-service';
import { getMyConnections } from '@/lib/connections/connections-service';

export default function PlayerDashboard() {
  const { user, loading } = useRequireRole(['player']);
            ...
            <div className="space-y-3">
              <h1 className="text-5xl font-bold">
                <span className="bg-gradient-to-r from-amber-400 via-amber-500 to-orange-500 bg-clip-text text-transparent">
                  Hey, {user.role || 'Player'}!
                </span>
                <span className="ml-3 inline-block animate-bounce">👋</span>
              </h1>
              <p className="text-gray-400 text-base max-w-2xl">
                Ready to improve your chess skills. Let's get started!
              </p>
            </div>
      ...
      <main className="relative max-w-7xl mx-auto px-8 py-12 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="relative">
              ...
              <div className="grid grid-cols-3 gap-6">
                <div className="group/stat relative p-6 bg-white/10 hover:bg-white/15 backdrop-blur-sm rounded-2xl border border-white/20 transition-all duration-300 shadow-lg">
                  ...
                  <p className="text-gray-300 text-sm uppercase tracking-wider relative">Total Assignments</p>
                </div>
                ...
                <div className="group/stat relative p-6 bg-white/10 hover:bg-white/15 backdrop-blur-sm rounded-2xl border border-white/20 transition-all duration-300 shadow-lg">
                  ...
                  <p className="text-gray-300 text-sm uppercase tracking-wider relative">Completed</p>
                </div>
                ...
                <div className="group/stat relative p-6 bg-white/10 hover:bg-white/15 backdrop-blur-sm rounded-2xl border border-white/20 transition-all duration-300 shadow-lg">
                  ...
                  <p className="text-gray-300 text-sm uppercase tracking-wider relative">👨‍🏫 Coaches</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-5 space-y-6">
            {/* Quick Action Card 1 */}
            <button
              onClick={() => router.push('/player/assignments')}
            ...
            >
              ...
              <div className="relative flex items-start justify-between">
                <div>
                  ...
                  <h3 className="text-xl font-bold text-white mb-2 group-hover:text-amber-400 transition-colors">View Assignments</h3>
            ...
            {/* Quick Action Card 2 */}
            <button
              onClick={() => router.push('/player/connections')}
            ...
            >
              ...
              <div className="relative flex items-start justify-between">
                <div>
                  ...
                  <h3 className="text-xl font-bold text-white mb-2 group-hover:text-purple-400 transition-colors">My Coaches</h3>
                  <p className="text-gray-400 text-sm">Connect with your mentors</p>
                </div>
          ...
          <div className="relative">
            <div className="flex items-center justify-between mb-8">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-2xl font-bold bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent">Your Assignments</h2>
                  <p className="text-gray-500 text-sm">Practice and improve your chess skills</p>
                </div>
              </div>
              <button
                onClick={() => router.push('/player/assignments')}
            ...
            {statsLoading ? (
              ...
              <div className="space-y-4">
                {recentAssignments.map((assignment) => {
                  const isOverdue = assignment.due_date && new Date(assignment.due_date) < new Date() && assignment.status === 'assigned';
                  const contentType = assignment.content?.type || 'lesson';
                    ...
                    >
                      ...
                      <div className="relative flex items-start justify-between gap-4">
                        <div className="flex items-start gap-4 flex-1 min-w-0">
                          <div className={`w-12 h-12 ${contentType === 'lesson' ? 'bg-blue-500/20' : 'bg-purple-500/20'} rounded-xl flex items-center justify-center flex-shrink-0`}>
                            <span className="text-2xl">{contentType === 'lesson' ? '📚' : '🧩'}</span>
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-2">
                              <h3 className="text-white font-semibold text-lg truncate">{assignment.content?.title || 'Untitled'}</h3>
                              <span className={`px-2 py-1 text-xs font-medium rounded-lg ${
                                contentType === 'lesson' 
                                  ? 'bg-blue-500/20 text-blue-400 border border-blue-500/30'
                                  : 'bg-purple-500/20 text-purple-400 border border-purple-500/30'
                              }`}>
                                {contentType === 'lesson' ? '📖 Lesson' : '🎯 Puzzle'}
                              </span>
                            </div>
                            <div className="flex items-center gap-4 text-sm">
                              {assignment.coach && (
                                <span className="text-gray-400">
                                  👨‍🏫 {assignment.coach.display_name || assignment.coach.email}
                                </span>
                              )}
                              {assignment.due_date && (
                        ...
                        <div className="flex items-center gap-3 flex-shrink-0">
                          <span className={`px-3 py-1 text-xs font-medium rounded-lg ${
                            ...
                            {assignment.status === 'completed' ? '✓ Done' : isOverdue ? '⚠ Overdue' : '⏳ Pending'}
                          </span>
              ...
              <div className="relative py-20">
                ...
                <div className="relative text-center">
                  ...
                  <p className="text-gray-400 max-w-md mx-auto">Your coach will assign practice materials to help you improve your chess skills.</p>
                </div>
              </div>
            )}
          </div>
